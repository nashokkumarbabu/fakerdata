...
const getFakeSchema = async (entityDoc) => {
    ...
    jsonata("*.versions[version=1].schema.{ 'type': 'object','properties':  {`field`:{'type' :typebindings[language='default'].dataType,'faker' :typebindings[language='FAKER'].dataType} }}").evaluate(entityDoc, {}, (error, result) => {
        if(error) {
          console.error(error);
          return;
        }
