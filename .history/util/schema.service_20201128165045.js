...
const getFakeSchema = async (entityDoc) => {
    ...
    await jsonata("{ 'type' : 'object','properties': *.schema{`field`:{'type' :typebindings[language='default'].dataType,'faker' :typebindings[language='FAKER'].dataType} }}").evaluate(entityDoc, {}, (error, result) => {
        if(error) {
          console.error(error);
          return;
        }
