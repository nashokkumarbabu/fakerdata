...
//     },
//   };
console.log(JSON.stringify(entityDoc._doc.versions)+" :: entityDoc");
entityDoc = entityDoc._doc;
  let schema = {};
  schema.type = "object";
  let entityVersion = entityDoc._doc.versions.filter((entityVersion) => {
  ...
  ).evaluate(entityDoc._doc, {}, (error, result) => {
    if (error) {
      console.error(error);
      return;
    }
