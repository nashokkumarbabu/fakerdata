...
const { getFakeSchema,getFakeUUID } = require("./util/schema.service");
...
app.post("/generatedata/:entity", async (req, res) => {
  ...
  const entityQuery = Entity.findOne({ name: entity }, function (err, doc) {
    if (err) {
      ...
      getFakeSchema(doc).then(function (result) {
        console.log(result);
        Entity.updateOne(
          { name: entity },
          { data: result },
          function (err, updateResult) {
            if (err) {
              ...
              res.send("Generated data successfully")
            }
          }
        );
      });
    ...
    if (req.method == "GET") {
      ...
      const entityQuery = Entity.findOne({ name: entity }, function (err, doc) {
        if (err) {
          ...
          reqEntity['_fid'] = getFakeUUID;
          console.log(Object.keys(reqEntity))
          dataArray.push(reqEntity);
          Entity.updateOne(
            { name: entity },
            { data: dataArray },
            function (err, result) {
